...
class HardhatNetworkProvider extends events_1.EventEmitter {
    constructor(_hardfork, _networkName, _chainId, _networkId, _blockGasLimit, _throwOnTransactionFailures, _throwOnCallFailures, _automine, _intervalMining, _logger, _genesisAccounts = [], _artifacts, _allowUnlimitedContractSize = false, _initialDate, _experimentalHardhatNetworkMessageTraceHooks = [], _forkConfig, _forkCachePath) {
        super();
    ...
    async _sendWithLogging(method, params = []) {
        try {
            ...
            if (!this._logger.isLoggedError(err)) {
                if (err instanceof errors_1.HardhatNetworkProviderError) {
                    this._logger.printEmptyLine();
