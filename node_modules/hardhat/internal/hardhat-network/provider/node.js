...
class HardhatNode extends events_1.default {
    ...
    static async create(config) {
        ...
        const txPool = new TxPool_1.TxPool(asPStateManager_1.asPStateManager(stateManager), new ethereumjs_util_1.BN(blockGasLimit), common);
        ...
        const node = new HardhatNode(vm, asPStateManager_1.asPStateManager(stateManager), blockchain, txPool, automine, initialBlockTimeOffset, genesisAccounts, tracingConfig);
        return [common, node];
    }
    ...
    async mineBlock(timestamp, sentTxHash) {
        ...
        try {
            ...
            if (err === null || err === void 0 ? void 0 : err.message.includes("sender doesn't have enough funds")) {
                throw new errors_2.InvalidInputError(err.message);
            }
    ...
    async runCall(call, blockNumberOrPending) {
        let tx;
        const result = await this._runInBlockContext(blockNumberOrPending, async () => {
            const account = await this._stateManager.getAccount(call.from);
            const nonce = new ethereumjs_util_1.BN(account.nonce);
            tx = await this._getFakeTransaction(Object.assign(Object.assign({}, call), { nonce }));
            return this._runTxAndRevertMutations(tx, blockNumberOrPending, false, true);
        });
    ...
    async estimateGas(txParams, blockNumberOrPending) {
        const tx = await this._getFakeTransaction(Object.assign(Object.assign({}, txParams), { gasLimit: this.getBlockGasLimit() }));
        ...
        //  the first block.
        const result = await this._runInBlockContext(blockNumberOrPending, () => this._runTxAndRevertMutations(tx, blockNumberOrPending, true));
    ...
    async _mineBlockWithPendingTxs(blockTimestamp, sentTxHash) {
        const block = await this._getNextBlockTemplate(blockTimestamp);
        const bloom = new bloom_1.default();
        const results = [];
        const receipts = [];
        const traces = [];
        const receiptTrie = new merkle_patricia_tree_1.default();
        ...
        while (gasLeft.gte(minTxFee) && tx !== undefined) {
            const shouldThrow = sentTxHash === ethereumjs_util_1.bufferToHex(tx.hash());
            ...
            if (txResult !== null) {
                ...
                await util_1.promisify(receiptTrie.put).bind(receiptTrie)(ethereumjs_util_1.rlp.encode(receipts.length - 1), ethereumjs_util_1.rlp.encode(Object.values(receipt)));
        ...
        block.header.gasUsed = ethereumjs_util_1.toBuffer(blockGasLimit.sub(gasLeft));
    ...
    async _runTx(tx, block, gasLeft, shouldThrow) {
    ...
    async _assignBlockReward() {
        ...
        miner.balance = ethereumjs_util_1.toBuffer(new ethereumjs_util_1.BN(miner.balance).add(blockReward));
    ...
    _getBlockReward() {
        return new ethereumjs_util_1.BN(this._vm._common.param("pow", "minerReward"));
    }
    _createReceipt(txResult, cumulativeGasUsed) {
        var _a;
        return {
            status: txResult.execResult.exceptionError === undefined ? 1 : 0,
            gasUsed: ethereumjs_util_1.toBuffer(cumulativeGasUsed),
    ...
    async _updateTransactionsRoot(block) {
     ...
     * makes.
     */
    async _runTxAndRevertMutations(tx, blockNumberOrPending, calledToEstimateGas = false, 
